...
import { useSubprojectEnvironment } from '@/hooks/useSubprojectEnvironment';
import { useSubprojectVariables } from '@/hooks/useSubprojectVariables';
import { useUser } from '@/hooks/useUser';
import { signOutAction } from '@/lib/auth/actions';
import {
	deployToSubprojectEasypanelAction,
	updateSubprojectEnvironmentEasypanelAction,
} from '@/lib/projects/subproject-actions';

interface Props {
	params: Promise<{ projectId: string; subprojectId: string; envId: string }>;
}

export default function SubprojectEnvironmentPage({ params }: Props) {
	const { projectId, subprojectId, envId } = use(params);
	const { user } = useUser();
	const {
		variables,
		isLoading,
		...
		updateVariable,
		deleteVariable,
	} = useSubprojectVariables(projectId, subprojectId, envId);
	const { environment } = useSubprojectEnvironment(
		projectId,
		subprojectId,
		envId,
	);
	...
	const saveEpConfig = useCallback(async () => {
		setEpSaving(true);
		setEpSaveMsg(null);
		const result = await updateSubprojectEnvironmentEasypanelAction(
			projectId,
			subprojectId,
			envId,
			{
	...
	}, [projectId, subprojectId, envId, epUrl, epToken, epService]);

	const deploy = useCallback(async () => {
		setEpDeploying(true);
		setEpDeployResult(null);
		const result = await deployToSubprojectEasypanelAction(
			projectId,
			subprojectId,
			envId,
		);
